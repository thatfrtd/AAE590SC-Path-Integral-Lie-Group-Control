function [g_k, twist_k] = one_step_euler_maruyama_lie_group(f, B, g_0, u_k, sigma)
...
for i = 1 : (numel(t_k) - 1)
    g_k(i + 1) = g_k(i).Exp(twist_k(:, i) * delta_t); % reconstruction equation
    twist_k(:, i + 1) = twist_k(:, i) + f(t_k(i), g_k(i), twist_k(:, i)) * delta_t + B(t_k(i), g_k(:, i)) * (u_k(:, i) * delta_t + sigma / sqrt(delta_t) * w_k(:, i));
end
end