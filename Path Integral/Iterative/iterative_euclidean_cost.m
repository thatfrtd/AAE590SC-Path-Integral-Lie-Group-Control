function [L, cost_t, cost_exit] = iterative_euclidean_cost(xtraj, vtraj, xtarg, vtarg, u_traj, dt, R, options)
arguments
    xtraj
    vtraj
    xtarg
...
cost_t = squeeze(sum(dot(u_traj, pagemtimes(R, u_traj))) * dt);% + diff_cost * diff_multiplier;
